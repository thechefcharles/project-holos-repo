---
name: "source-command-sync-notion"
description: "Sync repo work to Notion (Task Board, Decisions Log, trackers) — part of Definition of Done"
---

# source-command-sync-notion

Use this skill when the user asks to run the migrated source command `sync-notion`.
...
Read `/decisions.md` and compare against last sync marker (`.Codex/.notion-synced`).
...
```bash
git rev-parse HEAD > .Codex/.notion-synced
git add .Codex/.notion-synced
...
```bash
if [ -f .Codex/.notion-synced ]; then
  LAST_SYNCED=$(cat .Codex/.notion-synced)
else
  LAST_SYNCED="none"
fi
