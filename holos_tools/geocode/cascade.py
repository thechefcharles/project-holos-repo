...
class GeocodeCascade:
    ...
    def stage_1_address_point(self, norm_text: str, parsed, raw_text: str) -> Optional[GeocodeResult]:
        ...
        if result:
            ...
            return GeocodeResult(
                ...
                score=0.97,
                reason=f"Exact match: {parsed['number']} {parsed['street']}"
            )
    ...
    def stage_2_centerline(self, norm_text: str, parsed, raw_text: str) -> Optional[GeocodeResult]:
        ...
        if result:
            ...
            # Try left side
            if row["from_house_num_l"] <= house_num <= row["to_house_num_l"]:
                # Linear interpolation
                fraction = (house_num - row["from_house_num_l"]) / (row["to_house_num_l"] - row["from_house_num_l"] or 1)
                ...
                return GeocodeResult(
                    ...
                    coordinates=(lon, lat),
                    score=0.88,
                    reason=f"Interpolated on centerline: {parsed['street']}"
                )

            # Try right side
            if row["from_house_num_r"] <= house_num <= row["to_house_num_r"]:
                fraction = (house_num - row["from_house_num_r"]) / (row["to_house_num_r"] - row["from_house_num_r"] or 1)
                lon = row["start_lon"] + fraction * (row["end_lon"] - row["start_lon"])
                lat = row["start_lat"] + fraction * (row["end_lat"] - row["start_lat"])
